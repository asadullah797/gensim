        ...
        * `Matt Kusner et al. "From Word Embeddings To Document Distances"
        ...
        for i, t1 in dictionary.items():
            ...
            for j, t2 in dictionary.items():
                ...
                # Compute Euclidean distance between word vectors.
                #distance_matrix[i, j] = distance_matrix[j, i] = sqrt(np_sum((self[t1] - self[t2])**2))
                # Compute the PIP loss between word vectors;
                distance_matrix[i, j] = distance_matrix[j, i] = sqrt(np_sum((self[t1].dot(self[t1].T) - self[t2].dot(self[t2].T))**2))
